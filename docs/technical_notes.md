...
- B1全球センサス（134万地点）は`docs/related_work_and_storyline.md`で「進行中」と
  ...
  [NEXT_STEPS.md](../NEXT_STEPS.md)参照）。2026-07-10にDEGURBA層化サンプリング
  （[sampling/README.md](../sampling/README.md)）に置き換えて実行中
- **`ops run`長時間プロセスのRSS変動**（2026-07-11観察、原因未確定）: バッチ実行中、
  wsl上で13プロセス合計のRSSが数時間で数十GB規模まで増え空きメモリが逼迫する場面が
  あった。当初「`fetch.py`の`Fetcher`/`CacheFetcher`が`duckdb.connect()`を`__init__`で
  1回だけ生成しプロセス寿命中使い回すため、DuckDB内部キャッシュが単調増加するリーク」
  と推測したが、再起動後に短時間隔で追跡すると**同一プロセスのRSSが増加も減少もする**
  （例: 45分で2165MB→1431MBに減少した例あり）ことを確認し、単調リーク説は再考が必要。
  実際には**処理中の地点ごとのデータ量差（建物・道路が密な実データ地点 vs 空のbbox）に
  よる一時的な作業セットの変動**である可能性が高い。ただし複数プロセスの作業セットが
  同時に膨らむタイミングが重なると空きメモリを圧迫しうる。
  **暫定対処**: 空きメモリが逼迫したら該当マシンの全プロセスをkillして同じ
  `--start`/`--end`で再起動すれば安全に復旧する（`done`はスキップ、`failed`は再試行
  されるだけで再計算コストは軽い）。**要継続調査**: 真因（DuckDBキャッシュか単純な
  作業セット変動か）の切り分け
