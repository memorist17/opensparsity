...
Horvitz-Thompson型: `design_weight = 層の推定全体セル数 / 層から実際に抽出した点数`

- 「層の推定全体セル数」は `extract_candidates.py` が記録した抽出率（クラスごとに一定）から
  逆算する（層別に国判定をかけ直して全数を数えるのは重すぎるため、無作為抽出である前提で
  推定する）
- **【2026-07-12修正】** 当初はglobal_v2に倣い `cos(lat)` を掛けていたが、これは誤りだった。
  cos(lat)補正は**経緯度グリッド**（緯度が上がると1マスの実面積が縮む）のための補正で、
  GHS-SMODは**Mollweide等積図法**（ESRI:54009、全セルが正確に1km²）なので不要。
  等積格子に掛けると高緯度の点を不当に軽く扱う歪みが逆に入る。層内の包含確率は一様
  なので、重みは層内一定が正しい。
- **注意**: 2026-07-10生成の `final_sample.csv` の `design_weight` 列は旧式
  （cos(lat)入り）の非推奨値。本番実行の完了後、実際に指標が取れた点（実現サンプル）に
  対して重みを再計算するので、分析ではそちらを使うこと（点の選び方自体は等積格子上の
  一様抽出なので無バイアス、列の値だけの問題）
